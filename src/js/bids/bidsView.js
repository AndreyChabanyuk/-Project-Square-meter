...
function renderCountBids(bids){
    let counter = []
    bids.forEach((item)=>{
        counter.push(item);        
    })
    const markup = `<span class = "ml-2">${counter.length}</span>`
    document.querySelector('.fa-file-alt').insertAdjacentHTML('afterbegin',markup)
